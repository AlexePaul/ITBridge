		...
		<template #right>
			<div class="hidden items-center gap-3 md:flex">
				<template v-if="!userStore.user">
					<UButton label="Login" size="md" color="primary" variant="outline" @click="navigateTo('/auth/login')"/>
					<UButton label="Register" size="md" color="primary" variant="subtle" @click="navigateTo('/auth/register')"/>
				</template>
				<template v-else>
					<UButton label="Dashboard" size="md" color="primary"variant="outline" @click="navigateTo('/user/dashboard')"/>
