# Politica de confidențialitate

**Cum prelucrăm datele familiei tale și ale copilului tău.**

**Versiunea 0.1 · ciornă din 7 septembrie 2026 · neverificată de un avocat · nepublicată.**
Faptele marcate `[[…]]` lipsesc sau sunt propuneri; vezi [README](README.md).

---

## 1. Cine prelucrează datele și cum ne găsești

Operatorul datelor e **[[DENUMIREA LEGALĂ A ȘCOLII]]**, cu sediul în [[SEDIUL SOCIAL]], CUI
[[CUI]], care funcționează sub numele **IT Bridge School**. Cursurile se țin în București, pe
Strada Valea Oltului 73 (Sector 6) și pe Șoseaua București-Târgoviște 19A (Sector 1).

Pentru orice întrebare despre date, scrie la **[[EMAIL DEDICAT, de ex. date@itbridgeschool.com]]**
sau la office@itbridgeschool.com, ori sună la +40 732 273 347. Persoana care răspunde de protecția
datelor în școală e [[NUME]]. La dimensiunea școlii nu avem obligația unui responsabil cu protecția
datelor desemnat formal, dar cineva răspunde de asta și e persoana de mai sus.

Documentul ăsta se aplică site-ului `itbridgeschool.com`, formularului de programare la lecția de
probă și portalului de părinte. Se citește împreună cu
[Termenii și condițiile](termeni-si-conditii.md) și cu
[Politica de cookie-uri](politica-de-cookies.md).

## 2. Pe scurt

- **Ținem strictul necesar ca să ținem cursul, să te anunțăm și să facturăm.** Nu vindem date, nu
  punem reclame, nu urmărim ce faci pe site.
- **Despre copil ținem numele, data nașterii, grupa, prezența, absențele anunțate și lucrările
  făcute la curs.** Nu cerem CNP, nu fotografiem copiii și nu cerem date despre sănătate.
- **Datele stau în Uniunea Europeană**, pe servere din Frankfurt. Emailurile pleacă printr-un
  furnizor de trimitere; harta de pe site vine de la Google doar dacă accepți.
- **Mesaje de marketing primești numai dacă bifezi tu.** Factura, ora anulată sau lucrarea
  copilului nu sunt marketing și vin oricum.
- **La [[PROPUNERE: 12 luni]] de la retragerea din școală ștergem** contul, copiii și tot ce ține
  de ei. Evidența facturilor rămâne cât cere legea contabilității, fără datele tale de contact.
- **Poți cere oricând ce avem despre tine, corectarea sau ștergerea**, iar noi răspundem în cel
  mult o lună.

## 3. Situațiile în care prelucrăm date

Pentru fiecare situație: ce date, de ce, pe ce temei din Regulamentul (UE) 2016/679 („GDPR") și
cât timp — termenele sunt adunate în §7.

### 3.1 Când vizitezi site-ul

Paginile de prezentare nu cer nimic de la tine. Serverul care le livrează primește, ca orice
server, adresa IP, tipul browserului și pagina cerută, și le ține în loguri tehnice pentru
funcționare și securitate. Fonturile și imaginile sunt servite de pe domeniul nostru; nu folosim
unelte de analiză a traficului, pixeli de urmărire sau reclame.

Pe paginile celor două locații e o hartă Google. Ea se încarcă de la Google **numai după ce
accepți**, iar atunci Google primește adresa ta IP și poate pune cookie-urile lui — detaliile sunt
în politica de cookie-uri.

_Temei:_ interesul nostru legitim de a avea un site care funcționează și e sigur (art. 6 alin. 1
lit. f); pentru hartă, acordul tău (art. 6 alin. 1 lit. a).

### 3.2 Când ne scrii prin formularul de contact

Ne lași numele, un telefon sau un email, subiectul și mesajul. Formularul trimite un email la
office@itbridgeschool.com, prin furnizorul nostru de email; **nu se salvează nimic în platformă**,
mesajul trăiește doar în căsuța noastră de email. Ca să oprim roboții, serverul reține pentru 15
minute câte mesaje au venit de la aceeași adresă IP.

_Temei:_ cererea ta, adică pașii de dinaintea unui contract (art. 6 alin. 1 lit. b) sau, dacă nu
e vorba de înscriere, interesul legitim de a-ți răspunde (lit. f).

### 3.3 Când programezi o lecție de probă

Formularul „Lecție de probă" cere numele tău, cel puțin un email sau un telefon, prenumele, numele
și data nașterii copilului, și, opțional, ce a mai făcut copilul până acum și de unde ai auzit de
noi. Alegi locația și ora.

Cu ele scriem: o **cerere** (pe care o urmărim până se rezolvă), o **fișă de familie** fără cont,
**fișa copilului** și **rezervarea** locului la ora aleasă — locul e real, ca un alt părinte să nu-l
ia. Pe cerere, școala notează ce ține de urmărire: cine se ocupă, când te sunăm, ce s-a discutat,
și, dacă nu vă înscrieți, motivul pe scurt. Primești pe email confirmarea, un memento cu o zi
înainte și, dacă nu ajungeți, un singur mesaj ca să reprogramăm.

Data nașterii ne trebuie ca să-ți arătăm doar grupele de vârstă potrivită.

_Temei:_ pașii de dinaintea contractului, la cererea ta (art. 6 alin. 1 lit. b); urmărirea cererii
e interesul nostru legitim de a răspunde cui ne-a căutat (lit. f).

### 3.4 Când îți faci cont

**Ce ținem:** utilizator; parola, **numai ca hash** (nu o putem citi); prenume și nume; email;
telefon; adresă; persoana de contact pentru urgențe — nume, relația cu copilul, telefon; data la
care ai confirmat emailul; dacă și când școala a aprobat contul, sau motivul respingerii; sesiunile
deschise (un hash al tokenului, tipul browserului, când a fost deschisă, când expiră); linkurile de
confirmare trimise (hash și valabilitate, 48 de ore); bifa de marketing.

**De ce:** ca să știm cine ești și să te lăsăm în cont; ca să te sunăm dacă se întâmplă ceva la
curs — de asta telefonul, adresa și contactul de urgență sunt obligatorii; ca să emitem factura pe
numele și adresa ta; ca să-ți putem închide sesiunile dacă ți-a fost furat un dispozitiv.

**Persoana de urgență e altcineva decât tine.** Îi ținem numele și telefonul fiindcă ni le-ai dat
tu; te rugăm să-i spui că ne-ai dat numărul ei și pentru ce. Nu o contactăm decât într-o urgență cu
copilul.

_Temei:_ contractul cu tine (art. 6 alin. 1 lit. b); pentru numele și adresa de pe factură,
obligația legală de facturare (lit. c); pentru sesiuni și linkuri de confirmare, interesul legitim
de a ține contul în siguranță (lit. f).

### 3.5 Când copilul tău e înscris

**Despre copil ținem:** prenume, nume, data nașterii; grupa și istoricul înscrierilor — grupa,
de când până când, la probă sau activ, motivul ieșirii, data la care s-a semnat contractul; dacă e
pe o listă de așteptare, la ce grupă și de când; **prezența** la fiecare ședință — prezent sau
absent, și dacă a fost o ședință obișnuită, o recuperare sau o probă; **absențele anunțate** —
ședința, motivul pe scurt așa cum l-ai spus la birou, dacă anunțul a venit în termen, ora în care a
fost mutat pentru recuperare și cine a notat; **corecturile** numărului de ședințe pentru factură,
cu motiv, cine și când.

**De ce:** ca să ținem cursul — să știm cine e în ce grupă, cine a fost la oră, cine trebuie
recuperat — și ca să facturăm corect: factura numără ședințele ținute.

**Despre motivul absenței.** Notăm ce ne spui, pe scurt. Nu îți cerem motivul medical și te rugăm
să nu ni-l spui în detaliu: „nu poate veni" e suficient. Dacă totuși ne spui, rămâne între biroul
școlii și tine, nu apare nicăieri altundeva și îl ștergem odată cu restul.

_Temei:_ contractul de înscriere (art. 6 alin. 1 lit. b), pe care l-ai semnat în numele copilului;
organizarea grupelor pe vârste e interesul legitim al școlii de a ține un curs care se potrivește
copilului (lit. f). Copiii au sub 16 ani, deci tot ce ține de acordul lor se exercită prin tine.

### 3.6 Lucrările copilului

Ce face copilul la curs — proiecte Scratch, imagini, pagini web, documente, uneori scurte
înregistrări video ale ecranului — e salvat de profesor în dosarul copilului, pe calculatorul din
biroul școlii, și urcat de acolo în platformă. Ținem fișierele, numele lor original, tipul,
mărimea, data, o miniatură pe care o generăm noi din imagini, un titlu și, uneori, o descriere
scrisă de profesor, versiunile succesive, când și la ce adresă a fost trimisă lucrarea, și, dacă o
lucrare a fost atribuită greșit și corectată, urma corecturii.

Fișierele stau în spațiul nostru de stocare din Frankfurt, sub identificatori — numele copilului
nu apare în calea fișierului. Linkul din email se deschide doar din contul tău, iar descărcarea
trece printr-un link semnat, valabil 15 minute. Pe calculatorul din birou, dosarele poartă numele
copilului și un identificator, ca profesorul să știe unde salvează; rețeaua e a școlii și nu e
accesibilă din afară.

**Lucrarea ajunge numai la tine.** Nu o publicăm nicăieri fără un acord separat, dat de tine pentru
copilul acela și pentru acel scop, pe care îl poți retrage oricând, cu efect imediat. Nu fotografiem
copiii: fotografiem ecranul sau lucrarea.

_Temei:_ contractul (art. 6 alin. 1 lit. b) — livrarea lucrării e parte din curs; publicarea, dacă
o ceri vreodată, numai pe consimțământ (lit. a).

### 3.7 Facturi și plăți

**Ținem:** fiecare factură — suma, luna, data emiterii, starea; PDF-ul ei, cu numele și adresa ta;
fiecare plată — suma, data, metoda (numerar sau transfer), referința din extras, cine din școală a
înregistrat-o, o notă dacă a fost ceva de spus; reducerile — nume, tip, valoare, luna.

[[DE CONFIRMAT ÎNAINTE DE PUBLICARE: platforma generează azi PDF-ul facturii și îl păstrează.
Când emiterea trece prin SmartBill (E16 S2), documentul fiscal e emis și păstrat de SmartBill, iar
noi ținem referința; SmartBill primește atunci numele și adresa ta. Paragraful de mai jos se
activează în ziua aceea.]]

[[Facturile sunt emise prin SmartBill (Intelligent IT S.R.L., Sibiu), care primește numele,
adresa și suma, emite documentul fiscal și, unde legea cere, îl transmite în sistemul e-Factura al
ANAF. Platforma noastră ține referința documentului, nu îl produce.]]

**De ce:** ca să te facturăm și să ținem evidența — e contractul, și e o obligație legală: legea
contabilității ne cere să păstrăm documentele contabile, în prezent **5 ani**.

_Temei:_ contractul (art. 6 alin. 1 lit. b) și obligația legală (lit. c — Legea contabilității
nr. 82/1991 și Codul fiscal).

### 3.8 Mesajele pe care le trimitem

Tot ce trimitem pe email trece printr-o coadă din platformă, care ține **copia fiecărui mesaj**:
destinatar, subiect, text, starea livrării (trimis, în așteptare, eșuat, fără adresă) și motivul
unui eșec. Anunțurile școlii — către o grupă, o locație sau toată școala — sunt ținute cu textul,
audiența și numărul de destinatari. Trimiterea o face un furnizor de email (§5).

**Mesajele de serviciu** — confirmări de cont, factura, mementourile de plată, confirmarea plății,
ore anulate sau mutate, recuperarea, lista de așteptare, lucrarea trimisă, anunțurile de
organizare — pleacă fiindcă ai cont și copil înscris; nu depind de nicio bifă.

**Mesajele de marketing** pleacă numai cu **bifa ta**, care e implicit oprită și pe care o schimbi
din „Profil". Un anunț promoțional către o grupă sare peste familiile fără bifă; refuzul nu lasă
nicio urmă în afară de un număr.

_Temei:_ contractul (art. 6 alin. 1 lit. b) pentru mesajele de serviciu; consimțământul (lit. a)
pentru marketing, retractabil oricând, fără efect asupra restului.

### 3.9 Securitate și loguri

Serverul ține loguri tehnice cu adresa IP, tipul browserului, ora și ce s-a cerut, inclusiv
încercările de autentificare. Limitarea de rată numără cererile pe adresă IP timp de un minut, în
memorie. Tokenul de acces expiră în 15 minute; cel de reîmprospătare, în 7 zile; folosirea unui
token deja consumat închide toate sesiunile contului, fiindcă e semnul unui furt.

_Temei:_ interesul legitim de a proteja contul tău și platforma (art. 6 alin. 1 lit. f).

### 3.10 Listele interne ale școlii

Platforma pune în fața biroului câteva liste, o dată pe săptămână sau la cerere: copii cu trei
absențe la rând, facturi rămase neplătite, cereri de probă fără răspuns, grupe cu locuri goale.
Sunt calculate din datele de mai sus și **nu declanșează nimic** — nu se retrage niciun copil, nu
se schimbă niciun preț, nu pleacă niciun mesaj către familie. Sunt un motiv pentru un telefon,
dat de un om. Nu luăm decizii automate cu efect asupra ta (art. 22 GDPR nu e incident).

_Temei:_ interesul legitim de a conduce școala și de a observa la timp o familie care are nevoie
de o vorbă (art. 6 alin. 1 lit. f).

## 4. Datele copiilor: ce nu colectăm

- **CNP-ul** — nici al copilului, nici al tău. Factura nu îl cere.
- **Fotografii ale copilului.** Regula școlii e „se fotografiază lucrarea, nu copilul": fără fețe,
  fără copii în cadru.
- **Date despre sănătate**, alergii, diagnostice. Dacă profesorul trebuie să știe ceva ca să aibă
  grijă de copil, ni-l spui la telefon sau în persoană, și rămâne la profesor, nu în platformă.
- **Conturi pentru copii.** Copilul nu se autentifică nicăieri și nu primește niciun email.
- **Evaluări, note, observații.** Platforma nu le are.

Copiii au sub 16 ani. Contul e al părintelui, iar drepturile din §8 le exerciți tu, în numele
lor. Când copilul împlinește 16 ani și vrea să și le exercite singur, poate să ne scrie.

## 5. Cine mai vede datele

### 5.1 În școală

Profesorii și biroul — la momentul ăsta cei doi fondatori, care predau și administrează — văd
tot ce e în platformă, fiindcă tot ce e acolo e ceea ce au nevoie ca să țină cursul și să
factureze. Un părinte își vede doar propria familie; regula e impusă de server, nu de ecran, și
e verificată de teste la fiecare schimbare de cod.

Dosarele cu lucrări de pe calculatorul din birou sunt pe rețeaua școlii, accesibile de pe
calculatoarele din săli; [[DE CONFIRMAT: procedura de acces la ele]].

### 5.2 Furnizori care prelucrează în numele nostru

| Cine                                         | Ce face pentru noi                                                                  | Unde prelucrează                                                               |
| -------------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Amazon Web Services EMEA SARL                | găzduiește serverul, baza de date și fișierele lucrărilor                           | Frankfurt, Germania (UE)                                                       |
| Vercel Inc.                                  | găzduiește site-ul public și formularul de contact                                  | rețea globală; sediul în SUA [[de verificat regiunea funcțiilor și logurilor]] |
| Resend, Inc.                                 | trimite emailurile — de serviciu, de marketing și pe cele din formularul de contact | SUA [[de verificat regiunea UE din contul Resend]]                             |
| [[SmartBill — Intelligent IT S.R.L., Sibiu]] | emite și păstrează facturile [[de la E16 S2]]                                       | România (UE)                                                                   |
| Google Ireland Ltd.                          | harta de pe paginile locațiilor, numai după acordul tău                             | Irlanda / SUA                                                                  |
| [[CABINETUL DE CONTABILITATE]]               | contabilitatea școlii                                                               | România (UE)                                                                   |

Cu fiecare avem sau vom avea un acord de prelucrare a datelor, conform art. 28 GDPR
[[DE CONFIRMAT: lista acordurilor semnate — E07 S7]]. Ei prelucrează doar ce le dăm și doar
pentru ce le dăm.

### 5.3 Alții

- **Banca** — când plătești prin transfer, banca ta și a noastră văd plata; sunt operatori
  independenți, nu prelucrează pentru noi.
- **Autorități** — ANAF primește facturile prin e-Factura, unde legea o cere; alte autorități, doar
  la cerere legală.

**Nu vindem date, nu le dăm pentru reclame și nu le transmitem altor școli sau firme.**

## 6. Transferuri în afara Uniunii Europene

Datele stau în UE. Trei furnizori au sediul în Statele Unite — Vercel, Resend și Google — și pot
prelucra acolo părți din date (adresa IP la vizitarea site-ului, adresa de email și conținutul
mesajelor la trimitere). Transferul se face pe baza [[Cadrului UE–SUA de protecție a datelor
(Data Privacy Framework), în care furnizorul e certificat, sau a clauzelor contractuale standard
ale Comisiei Europene — de verificat pentru fiecare dintre cei trei]]. La cerere îți dăm o copie
a garanțiilor.

## 7. Cât timp păstrăm

Retragerea e un fapt pe care școala îl știe — ne anunți, sau constatăm — și de la data ei încep
termenele. Nu ștergem nimic „pentru inactivitate": o familie care ia o pauză de o vacanță nu
dispare.

| Ce                                                                                                    | Cât timp                                                                                                                                                           |
| ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Contul, datele de contact, persoana de urgență, bifa de marketing                                     | [[PROPUNERE: 12 luni]] de la retragerea ultimului copil sau de la închiderea contului, apoi șterse                                                                 |
| Copiii: nume, data nașterii, înscrieri, lista de așteptare, prezența, absențele anunțate, corecturile | același termen; apoi șterse. Ședința rămâne în orar, fără copil                                                                                                    |
| Lucrările: fișierele, miniaturile, urma trimiterii                                                    | același termen; apoi șterse din stocarea noastră. Pe calculatorul din birou: [[DE DECIS: procedura școlii]]                                                        |
| Cererea de probă și fișa creată de ea, dacă familia nu s-a înscris                                    | [[PROPUNERE: 12 luni]] de la ultima discuție, apoi șterse                                                                                                          |
| Facturile și plățile                                                                                  | **5 ani** de la închiderea exercițiului financiar (Legea contabilității), la noi și la SmartBill; la termenul de mai sus rămân fără datele de contact ale familiei |
| Copiile mesajelor trimise                                                                             | [[PROPUNERE: 12 luni]] de la trimitere                                                                                                                             |
| Sesiunile de autentificare                                                                            | cel mult 7 zile; cele expirate se șterg automat                                                                                                                    |
| Linkurile de confirmare a emailului                                                                   | 48 de ore                                                                                                                                                          |
| Logurile tehnice ale serverului                                                                       | [[PROPUNERE: 30 de zile]]                                                                                                                                          |
| Mesajele din formularul de contact                                                                    | în căsuța noastră de email, cât e nevoie ca să răspundem, cel mult [[PROPUNERE: 24 de luni]]                                                                       |
| Cookie-urile                                                                                          | vezi politica de cookie-uri                                                                                                                                        |

Ștergerea la cerere (§8) taie prin termenele de mai sus: ce nu suntem obligați legal să păstrăm se
șterge la cerere, nu la termen.

## 8. Drepturile tale

Ai dreptul:

- **să afli** ce date avem despre tine și despre copil, și să primești o copie;
- **să corectezi** ce e greșit — datele de contact și ale copiilor le corectezi singur din portal;
- **să ceri ștergerea**, cu excepția a ce trebuie să păstrăm prin lege (facturile);
- **să ceri restrângerea** prelucrării cât timp verificăm o obiecție;
- **să primești datele** pe care ni le-ai dat într-un format care se poate citi de un program, sau
  să le trimitem altui operator;
- **să te opui** prelucrărilor bazate pe interesul nostru legitim (§3.1, 3.3, 3.9, 3.10);
- **să retragi consimțământul** pentru marketing sau pentru hartă, oricând, fără ca retragerea să
  afecteze ce s-a făcut înainte.

**Cum:** scrii la [[EMAIL DEDICAT]] sau la office@itbridgeschool.com, de pe adresa din cont, ori
ne suni. Dacă nu te putem recunoaște, îți cerem să confirmi identitatea într-un fel care nu ne
obligă să ținem alte date. Răspundem în **cel mult o lună**; dacă cererea e complicată, putem
prelungi cu două luni și îți spunem de ce. E gratuit.

**Din portal, fără să ne scrii:** îți schimbi datele, adaugi sau corectezi copiii, pornești sau
oprești marketingul, îți închizi sesiunile. Ștergerea contului se cere la școală, ca să putem
păstra ce cere legea și să ștergem restul.

**Dacă nu ești mulțumit** de răspuns, te poți adresa Autorității Naționale de Supraveghere a
Prelucrării Datelor cu Caracter Personal (ANSPDCP): B-dul G-ral. Gheorghe Magheru 28-30, Sector 1,
010336 București; anspdcp@dataprotection.ro; www.dataprotection.ro. Te poți adresa și instanței.

## 9. Cum protejăm datele

- Conexiunea cu platforma e criptată (HTTPS). Parola e stocată doar ca hash.
- Un părinte vede doar propria familie; regula e în server, testată la fiecare schimbare.
- Tokenurile de acces sunt scurte (15 minute), cele de reîmprospătare se rotesc la fiecare
  folosire, iar refolosirea unuia consumat închide toate sesiunile contului.
- Fișierele se descarcă prin linkuri semnate, scurte, ca atașament, nu se deschid în pagină. Calea
  unui fișier nu conține numele copilului.
- În cookie-uri nu ținem date despre tine, doar tokenurile de sesiune.
- Serverul din Frankfurt accesează stocarea prin rolul lui, fără chei statice; secretele nu stau în
  cod. [[DE CONFIRMAT la publicare: copii de siguranță ale bazei — E04 S4.]]
- Dacă avem vreodată o breșă care îți pune datele în pericol, anunțăm ANSPDCP în 72 de ore și pe
  tine, dacă riscul pentru tine e ridicat.

## 10. Modificări

Când se schimbă platforma, se schimbă și documentul ăsta. Fiecare versiune are număr și dată, iar
versiunile vechi rămân disponibile. Pentru schimbări care te privesc — o categorie nouă de date, un
furnizor nou, un termen mai lung — te anunțăm pe email înainte să intre în vigoare.
